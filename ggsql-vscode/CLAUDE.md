...
The kernel build is the expensive step — around 16 min cold against 2.5 min warm — and it is uncached anyway. A `Swatinem/rust-cache` entry for it runs about 456 MB against a 10 GB repo-wide limit that other workflows have already spent most of; `build.yaml`'s own cargo cache alone is over 5 GB. The budget is shared and the job is not on the critical path for a PR, so the build is paid in wall-clock instead. If that trade is ever revisited, the thing to add back is a `shared-key` with `save-if: github.ref == 'refs/heads/main'`, so PRs restore one entry rather than each writing their own — see [`/.github/workflows/test-extension.yaml`](../.github/workflows/test-extension.yaml).
