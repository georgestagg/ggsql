...
WHERE category AS 'Electronics'
VISUALISE AS PLOT
DRAW line
    MAPPING sale_date AS x, revenue AS y
SCALE x SETTING type TO 'date'
LABEL title AS 'Electronics Revenue Over Time', x AS 'Date', y AS 'Revenue ($)'
```
...
LABEL title AS 'Total Revenue by Category', x AS 'Category', y AS 'Total Revenue ($)'
```
...
DRAW point
    ...
    SETTING size TO 30
SCALE x SETTING type TO 'date'
LABEL title AS 'Time Series with Points', x AS 'Date', y AS 'Value'
```
...
SCALE x SETTING type TO 'date'
LABEL title AS 'Metrics by Category', x AS 'Date', y AS 'Value'
```
...
SCALE x SETTING type TO 'date'
FACET WRAP region
LABEL title AS 'Electronics Sales by Region', x AS 'Date', y AS 'Revenue ($)'
```
...
LABEL title AS 'Monthly Revenue and Quantity by Region and Category', x AS 'Month', y AS 'Value'
```
...
COORD flip
LABEL title AS 'Total Revenue by Region', x AS 'Region', y AS 'Total Revenue ($)'
```
...
LABEL title AS 'Scatter Plot with Custom Axis Limits', x AS 'X', y AS 'Y'
```
...
COORD polar
LABEL title AS 'Revenue Distribution by Category'
```
...
DRAW point
    MAPPING month AS x, total_revenue AS y
SCALE x SETTING type TO 'date'
LABEL title AS 'Monthly Revenue Trends', x AS 'Month', y AS 'Revenue ($)'
```
...
LABEL title AS 'Revenue by Category and Region', x AS 'Category', y AS 'Revenue ($)'
```
