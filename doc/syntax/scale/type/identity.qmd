...
Since the values are used as-is, they are read exactly the way a literal given with [`SETTING`](../../clause/draw.qmd) is read, in the same unit and the same vocabulary: a column mapped to [`size`](../aesthetic/size.qmd) is a radius in points, one mapped to [`linewidth`](../aesthetic/linewidth.qmd) is a width in points, and one mapped to [`shape`](../aesthetic/shape.qmd) or [`linetype`](../aesthetic/linetype.qmd) holds the same names you would write as a setting (`'star'`, `'dashed'`). Data measured in something else needs converting in SQL first.
