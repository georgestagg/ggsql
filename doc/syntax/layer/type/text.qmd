...
### Parse
By default the label is read as markdown, so `'**Adelie**'` draws a bold *Adelie* rather than the asterisks around it.
Set `parse => false` to draw the label exactly as it is, markers and all.

The markdown flavour recognised is CommonMark plus a few extensions.
The most useful parts for a label are:
...
The Vega-Lite writer has no rich-text support and always draws the label literally, so a query meant for both writers should either avoid markdown in its labels or set `parse => false`.
